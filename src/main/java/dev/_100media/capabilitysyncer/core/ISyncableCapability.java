...
public interface ISyncableCapability extends INBTSavable<CompoundTag> {
    ...
    CapabilityStatusPacket createUpdatePacket(int id);

    default void sendUpdatePacketToPlayer(ServerPlayer serverPlayer) {
        getNetworkChannel().send(PacketDistributor.PLAYER.with(() -> serverPlayer), this.createUpdatePacket(serverPlayer.getId()));
    }
